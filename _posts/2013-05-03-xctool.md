...
To demonstrate how easy xctool is to use, we'll create a dummy sample app, MyApp, with a unit test target. We'll then both build the project and run its test cases via xctool. Compared to the previous required effort (especially when using Jenkins/Hudson), it is surprisingly simple.
